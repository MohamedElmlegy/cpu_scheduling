...
namespace WindowsFormsApp1
{
    ...
    public partial class Form5 : Form
    {
                ...
                case "Round Robin":
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            round_robin(output,  p_arrival, p_burst, this.quant, this.no_prc, ref this.avg_waiting_t);
                            round_robin_chart(output, p_arrival);
                            textBox5.Text = this.avg_waiting_t.ToString("0.00");
        ...
        private void round_robin(List<List<int>> output,  List<int> at, List<int> bt, int quant, int nop , ref double avg_wt)
        {

            // initlialize the variable name  
            int i, sum = 0, count = 0, y, wt = 0, tat = 0;
            int loopn = -1;
            int[] temp = new int[bt.Count];
            double  avg_tat;
            y = nop;
            for (i = 0; i < nop; i++)
            {
            ...
            for (sum = 0, i = 0; y != 0;)
            {
                ...
                {
                    sum = sum + temp[i];
                    output[loopn].Add( i+1);
                    output[loopn].Add(temp[i]);
                    temp[i] = 0;
                    count = 1;
                }
                else if (temp[i] > 0)
        ...
        private void round_robin_chart (List<List<int>> o, List<int> at)
        {

            chart1.Series[0].Points.AddXY(o[0][0], at[0], o[0][1]);

            for (int i = 1; i < o.Count; i++)
            {

                chart1.Series[0].Points.AddXY(o[i][0], o[i-1][1], o[i][ 1]);
